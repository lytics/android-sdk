package com.lytics.android

object Constants {
    const val SHARED_PREFERENCES_NAME = "_LYTICS_"
    const val KEY_IS_OPTED_IN = "is_opted_in"
    const val KEY_IS_GAID_ENABLED = "is_gaid_enabled"
    const val KEY_CURRENT_USER = "current_user"
    const val KEY_LAST_INTERACTION_TIME = "last_interaction_time"

    const val KEY_EVENT_NAME = "eventName"
    const val KEY_TIMESTAMP = "_ts"
    const val KEY_EVENT_TYPE = "_e"
    const val KEY_SESSION_START = "_sesstart"
    const val KEY_ADVERTISING_ID = "gaid"
    const val KEY_IDENTIFIERS = "identifiers"
    const val KEY_ATTRIBUTES = "attributes"
    const val KEY_CONSENT = "consent"
    const val KEY_PROPERTIES = "properties"

    const val KEY_SCREEN_EVENT_TYPE = "sc"
    const val KEY_SESSION_START_FLAG = 1
}
